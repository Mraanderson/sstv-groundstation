#!/bin/bash
# SSTV Groundstation Launcher (branch-aware, status line)
...
json_get() {
  grep -o "\"$1\"[[:space:]]*:[[:space:]]*[^,}]*" "$2" | sed 's/.*: *//;s/[",]//g'
}
status_line() {
  if ! have_repo; then
...
# --- Reclone ---
do_reclone() {
  read -rp "Backup before reclone? (y/n): " a
  [[ $a =~ [Yy] ]] && do_backup
  if [ -d "$APP_DIR" ]; then
    mv "$APP_DIR" "${APP_DIR}_old_$(date +%s)"
  fi
  git clone -b main "$REPO_URL" "$APP_DIR"
  ensure_venv
  msg "$GREEN" "Fresh clone ready"
}
...
admin_menu() {
  while :; do
    echo "---- Admin ----"
    status_line
    echo "1) Pull 2) Switch 3) Backup 4) Restore 5) Reclone 6) Back"
    read -rp "> " c
    case $c in
      1) pull_update ;;
      2) switch_branch ;;
...
main_menu() {
  while :; do
    echo "=== SSTV Launcher ==="
    status_line
    echo "1) Run 2) Start+TLE 3) Admin 4) Exit"
    read -rp "> " c
    case $c in
      1) run_local ;;
      2) start_with_tle ;;
