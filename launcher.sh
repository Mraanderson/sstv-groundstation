...
status_line(){ 
  if ! have_repo; then echo -e "${RED}Install: none${RESET}"; 
  else cd "$APP_DIR"; echo -e "${GREEN}Install: yes | Branch: $(git rev-parse --abbrev-ref HEAD)${RESET}"; fi
}
...
switch_branch(){ have_repo||{msg "$RED" "No install";return;}; cd "$APP_DIR"; git fetch --all --prune; status_line; mapfile -t b < <({git branch --format='%(refname:short)';git branch -r --format='%(refname:short)'|sed 's|origin/||';}|sort -u); select br in "${b[@]}";do [ -n "$br" ]&&{ git checkout "$br" 2>/dev/null||git checkout -b "$br" "origin/$br"; git pull --ff-only origin "$br"||true; status_line; break;};done; }
